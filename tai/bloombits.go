...
type BloomIndexer struct {
	...
	db  taidb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db taidb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := taidb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
