...
func testStatusMsgErrors(t *testing.T, protocol int) {
	...
	if protocol == eth63 {
		tests := []struct {
			...
			{
				code: StatusMsg, data: statusData63{uint32(protocol), 999, td, head.Hash(), genesis.Hash()},
				wantError: errResp(ErrNetworkIDMismatch, "999 (!= 19330)"),
			},
			{
				code: StatusMsg, data: statusData63{uint32(protocol), DefaultConfig.NetworkId, td, head.Hash(), common.Hash{3}},
		...
		tests := []struct {
			...
			{
				code: StatusMsg, data: statusData{10, DefaultConfig.NetworkId, td, head.Hash(), genesis.Hash(), forkid.NewID(pm.blockchain)},
				wantError: errResp(ErrProtocolVersionMismatch, "10 (!= %d)", protocol),
			},
			{
				code: StatusMsg, data: statusData{uint32(protocol), 999, td, head.Hash(), genesis.Hash(), forkid.NewID(pm.blockchain)},
				wantError: errResp(ErrNetworkIDMismatch, "999 (!= 19330)"),
			},
			{
				code: StatusMsg, data: statusData{uint32(protocol), DefaultConfig.NetworkId, td, head.Hash(), common.Hash{3}, forkid.NewID(pm.blockchain)},
				wantError: errResp(ErrGenesisMismatch, "0300000000000000 (!= %x)", genesis.Hash().Bytes()[:8]),
			},
		}
		for i, test := range tests {
...
func testRecvTransactions(t *testing.T, protocol int) {
	...
	pm, _ := newTestProtocolManagerMust(t, downloader.FullSync, 0, 0, nil, txAdded)
