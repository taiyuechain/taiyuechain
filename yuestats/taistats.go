...
// Service implements an Taiyuechain netstats reporting daemon that pushes local
...
func (s *Service) loop() {
	...
	for {
		...
		if err = s.reportSnailBlock(conn); err != nil {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["yue"]; info != nil {
...
// reportBlock retrieves the current chain head and reports it to the stats server.
func (s *Service) reportSnailBlock(conn *websocket.Conn) error {
	// Gather the block details from the header or block chain
	details := &snailBlockStats{
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.yue != nil {
		...
		if block == nil {
			block = s.yue.BlockChain().CurrentBlock()
		}
		header = block.Header()
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.yue != nil {
		sync := s.yue.Downloader().Progress()
		syncing = s.yue.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.yue.APIBackend.SuggestPrice(context.Background())
