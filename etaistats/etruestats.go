...
type Service struct {
	...
	etrue  *etai.Taiyuechain // Full Taiyuechain service if monitoring a full node
...
func New(url string, ethServ *etai.Taiyuechain, lesServ *les.LightEtrue) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["etrue"]; info != nil {
		network = fmt.Sprintf("%d", info.(*etai.NodeInfo).Network)
		protocol = fmt.Sprintf("etrue/%d", etai.ProtocolVersions[0])
	} else {
