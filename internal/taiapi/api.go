...
var (
	LocalTxMetrics = metrics.NewRegisteredMeter("yue/prop/local_tx/in", nil)
)
...
//
// https://github.com/taiyuechain/taiyuechain/wiki/Management-APIs#personal_sign
...
func (s *PublicTransactionPoolAPI) Resend(ctx context.Context, sendArgs SendTxArgs, gasPrice *hexutil.Big, gasLimit *hexutil.Uint64) (common.Hash, error) {
	...
	for _, p := range pending {
		signer := types.NewSigner(p.ChainId())
...
func (s *PublicCertAPI) ListPermission(ctx context.Context, group_contract_Addr common.Address, permType uint8, blockNr rpc.BlockNumber) (map[string]interface{}, error) {
	...
	if permType == 1 {
		...
		//group
		if v, ok := pTable.GropPermi[group_contract_Addr]; ok {
			if v.WhiteMembers != nil {
				if v.WhiteMembers.Member != nil {
					for _, m := range v.WhiteMembers.Member {
						wmember = append(wmember, m.MemberID)
		...
		//contrct
		if v, ok := pTable.ContractPermi[group_contract_Addr]; ok {
			if v.WhiteMembers != nil {
				if v.WhiteMembers.Member != nil {
					for _, m := range v.WhiteMembers.Member {
						wmember = append(wmember, m.MemberID)
