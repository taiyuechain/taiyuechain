...
共用配置：
模板中默认配置都是共用配置，唯一需要修改是的Node.P2P.BootstrapNodes字段，我们可以通过`gtai`程序分别预生成4个节点的enode信息。

> gtai enode privkey
privkey: 为之前生成的4个节点的私钥。
执行4次endoe命名分别生成4个如下的enode信息，并将enode信息中`127.0.0.1:30303`改成4个节点实际对应的`IP:Port`信息。
```
gtai enode "c1581e25937d9ab91421a3e1a2667c85b0397c75a195e643109938e987acecfc"
...
独立配置：
`GTai.CommitteeKey`: 4个节点每个节点配置一个私钥，十六进制表示的字符串私钥。
`GTai.NodeCertFile`: 4个节点每个节点指定一个证书文件(证书文件完整路径)。
`GTai.Host`: 4个节点各自的IP地址(如果需要连接外网，则需要外网IP地址)。
