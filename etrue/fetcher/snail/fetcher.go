...
type announce struct {
	...
	header *types.SnailHeader // Header of the block partially reassembled (new protocol)
...
type headerFilterTask struct {
	...
	headers []*types.SnailHeader // Collection of headers to filter
...
type Fetcher struct {
	...
	importedHook       func(block *types.SnailBlock) // Method to call upon successful block import (both eth/61 and eth/62)
}
...
func (f *Fetcher) FilterHeaders(peer string, headers []*types.SnailHeader, time time.Time) []*types.SnailHeader {
	...
	for {
		...
		case <-completeTimer.C:
			...
			for peer, hashes := range request {
				...
				go f.completing[hashes[0]].fetchBodies(hashes, false, types.FetcherCall)
			}
			// Schedule the next fetch if blocks are still pending
		...
		case filter := <-f.headerFilter:
			...
			unknown, incomplete, complete := []*types.SnailHeader{}, []*announce{}, []*types.SnailBlock{}
			for _, header := range task.headers {
				...
				if announce := f.fetching[hash]; announce != nil && announce.origin == task.peer && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if f.getBlock(hash) == nil {
						...
						if header.FruitsHash == types.DeriveSha(types.Transactions{}) && header.SignHash == types.CalcSignHash([]*types.PbftSign{}) {
		...
		case filter := <-f.bodyFilter:
			...
			bodyFilterOutMeter.Mark(int64(len(task.fruits)))
			select {
			case filter <- task:
			case <-f.quit:
				return
...
func (f *Fetcher) enqueue(peer string, block *types.SnailBlock) {
	...
	if count > blockLimit {
		log.Debug("Discarded propagated snail block, exceeded allowance", "peer", peer, "number", block.Number(), "hash", hash, "limit", blockLimit)
	...
	if dist := int64(block.NumberU64()) - int64(f.chainHeight()); dist < -maxUncleDist || dist > maxQueueDist {
		log.Debug("Discarded propagated snail block, too far away", "peer", peer, "number", block.Number(), "hash", hash, "distance", dist)
