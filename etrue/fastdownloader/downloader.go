...
import (
	...
	etrue "github.com/taiyuechain/taiyuechain/etrue/types"
...
var (
	...
	errPeerNil                 = errors.New("Fast peer is nil")
)
...
func New(mode SyncMode, stateDb etruedb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer etrue.PeerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
func (d *Downloader) Synchronise(id string, head common.Hash, mode SyncMode, origin uint64, height uint64) error {
	...
	case errTimeout, errBadPeer, errStallingPeer,
		...
		log.Warn("Fast Synchronisation failed, dropping peer", "peer", id, "err", err)
		if d.dropPeer == nil {
...
func (d *Downloader) fetchParts(errCancel error, deliveryCh chan etrue.DataPack, deliver func(etrue.DataPack) (int, error), wakeCh chan bool,
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			log.Trace("Fast deliver", "type", kind, "pending", pending(), "count", packet.Stats())
			...
			for pid, fails := range expire() {
				if peer := d.peers.Peer(pid); peer != nil {
					...
					if fails > 2 {
						...
						peer.GetLog().Debug("Stalling delivery,fast dropping", "type", kind)
						if d.dropPeer == nil {
							// The dropPeer method is nil when `--copydb` is used for a local copy.
							// Timeouts can occur if e.g. compaction hits at the wrong time, and can be ignored
...
func (d *Downloader) processHeaders(origin uint64, pivot uint64) error {
	...
	defer func() {
		if len(rollback) > 0 {
			...
			log.Warn("Fast Rolled back headers", "count", len(hashes),
	...
	for {
		...
		case headers := <-d.headerProcCh:
			log.Trace("processHeaders fast Terminate", "headers", len(headers), "PendingBlocks", d.queue.PendingBlocks(), "PendingReceipts", d.queue.PendingReceipts())
			...
			d.syncStatsLock.Unlock()
			log.Debug("processHeaders fast over", "headers", len(headers), "headerProcCh", len(d.headerProcCh), "origin", origin)
...
func (d *Downloader) importBlockResults(results []*etrue.FetchResult) error {
	...
	log.Debug("Fast Inserting downloaded fast chain", "items", len(results),
		...
		if P != nil {
			...
			case <-stateSync.Done():
				...
				if err := d.commitPivotBlock(P); err != nil {
					log.Error("SyncCommitHead state ", "err", err)
					return err
				}
...
func (d *Downloader) commitPivotBlock(result *etrue.FetchResult) error {
	...
	log.Debug("Fast Committing sync pivot as new head", "number", block.Number(), "hash", block.Hash())
...
func (d *Downloader) commitFastSyncData(results []*etrue.FetchResult) error {
	...
	// Retrieve the a batch of results to import
	first, last := results[0].Fheader, results[len(results)-1].Fheader
	log.Debug("Fast Inserting sync blocks", "items", len(results),
		"firstnum", first.Number, "firsthash", first.Hash(),
...
func (d *Downloader) DeliverHeaders(id string, headers []*types.Header, call uint32) (err error) {
	watch := help.NewTWatch(3, fmt.Sprintf("peer: %s, handleMsg DeliverHeaders, call: %d header %d", id, call, len(headers)))
	defer func() {
		watch.EndWatch()
		watch.Finish("end")
	}()
...
func (d *Downloader) DeliverBodies(id string, transactions [][]*types.Transaction, signs [][]*types.PbftSign, infos [][]*types.CommitteeMember, call uint32) (err error) {
	watch := help.NewTWatch(3, fmt.Sprintf("peer: %s, handleMsg DeliverBodies, call: %d header %d", id, call, len(transactions)))
	defer func() {
		watch.EndWatch()
		watch.Finish("end")
	}()
...
func (d *Downloader) qosTuner() {
	for {
		...
		log.Debug("Fast Recalculated downloader QoS values", "rtt", rtt, "confidence", float64(conf)/1000000.0, "ttl", d.requestTTL())
		select {
		case <-d.quitCh:
			return
		case <-time.After(rtt):
...
func (d *Downloader) qosReduceConfidence() {
	...
	log.Debug("Fast Relaxed downloader QoS values", "rtt", rtt, "confidence", float64(conf)/1000000.0, "ttl", d.requestTTL())
}
