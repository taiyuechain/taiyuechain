...
func importPreimages(ctx *cli.Context) error {
	...
	stack := makeFullNode(ctx)
	diskdb := utils.MakeChainDatabase(ctx, stack).(*etaidb.LDBDatabase)
...
func copyDb(ctx *cli.Context) error {
	...
	sdl := downloader.New(syncmode, chainDb, new(event.TypeMux), nil, nil, nil, fdl)
	...
	speer := downloader.NewFakePeer("local", db, sdl)
	...
	//	return err
	//}
	if err = sdl.Synchronise("local", currentHeader.Hash(), big.NewInt(0), syncmode); err != nil {
		return err
	...
	// Create a source peer to satisfy downloader requests from
	db, err := etaidb.NewLDBDatabase(ctx.Args().First(), ctx.GlobalInt(utils.CacheFlag.Name), 256)
	if err != nil {
		return err
	}
	...
	fmt.Println("Compacting entire database...")
	//TODO
	if err = chainDb.(*etaidb.LDBDatabase).LDB().CompactRange(util.Range{}); err != nil {
		utils.Fatalf("Compaction failed: %v", err)
	}
	fmt.Printf("Compaction done in %v.\n\n", time.Since(start))
	return nil
