...
func SendP256Transtion(ip string) {
	...
	tx := types.NewP256Transaction(nonce, &to, nil, amount,
		new(big.Int).SetInt64(0), params.TxGas, new(big.Int).SetInt64(0), nil, nil)
...
func generateCreateContractTx(nonce uint64, amount *big.Int, fromcert []byte, chainID *big.Int) *types.Transaction {
	...
	return types.NewP256Transaction(nonce, nil, nil, amount, new(big.Int).SetInt64(0),
		6000000, new(big.Int).SetInt64(0), Databytes, nil)
}
func sendErc20TokenTx(nonce uint64, fromcert []byte, chainID *big.Int) *types.Transaction {
	...
	return types.NewP256Transaction(nonce, &to, nil, new(big.Int).SetInt64(0),
		...
		6000000, new(big.Int).SetInt64(1000), Databytes, nil)
}
...
func getBalanceValue(hex string, print bool) *big.Int {
	...
	if print {
		fmt.Println("yue_getBalance Ok:", " true ", balance.Div(balance, big.NewInt(1000000000000000000)), " value ", value, " hex ", hex)
	}
	return value
}
...
func getAccountBalance(client *rpc.Client, account string) string {
	var result string
	// get balance
	err := client.Call(&result, "yue_getBalance", account, "latest")
	if err != nil {
		fmt.Println("yue_getBalance Error:", err)
		msg <- false
		return ""
	}
	return result
	...
	for {
		for i := 0; i < count; i++ {
			...
			if getBalance {
				...
				fmt.Println("yue_getBalance son address ", account[i], " result ", balance, " i ", i, " true ", balanceTrue.Div(balanceTrue, big.NewInt(1000000000000000000)))
				if balance.Cmp(average) >= 0 {
