...
func TestDeployContract(t *testing.T) {
	...
	chain, _ := core.GenerateChain(gspec.Config, genesis, pow, db, 10, func(i int, gen *core.BlockGen) {
		switch i {
		case 1:
			tx, _ := types.SignTx(types.NewContractCreation(gen.TxNonce(pAccount1), big.NewInt(30), 1000000, new(big.Int).SetUint64(1), bytecode), signer, pKey1)
			gen.AddTx(tx)
		...
		case 2:
			...
			tx1, _ := types.SignTx(types.NewTransaction(gen.TxNonce(pAccount1), contractAddress, big.NewInt(1000), 500000, nil, input), signer, pKey1)
			gen.AddTx(tx1)
