...
func TestAllCaCert(t *testing.T) {
	...
	executable := func(number uint64, gen *core.BlockGen, fastChain *core.BlockChain, header *types.Header, statedb *state.StateDB) {
		...
		cert3 := []byte{126}
		cert44 := []byte{127}
		sendAddCaCertTranscation(number, gen, saddr1, cert, skey1, signer, statedb, fastChain, abiCA, nil, p2p2Byte)
		sendAddCaCertTranscation(number, gen, saddr1, cert1, skey1, signer, statedb, fastChain, abiCA, nil, p2p2Byte)
		sendAddCaCertTranscation(number, gen, saddr1, cert2, skey1, signer, statedb, fastChain, abiCA, nil, p2p2Byte)
		sendAddCaCertTranscation(number, gen, saddr1, cert3, skey1, signer, statedb, fastChain, abiCA, nil, p2p2Byte)
		sendGetCaCertAmountTranscation(number, gen, saddr1, cert, skey1, signer, statedb, fastChain, abiCA, nil, p2p2Byte)
		sendIsApproveCACertTranscation(number, gen, saddr1, cert, skey1, signer, statedb, fastChain, abiCA, nil, p2p2Byte)

		sendMultiProposalTranscation(number, gen, saddr1, cert44, cert, true, skey1, signer, statedb, fastChain, abiCA, nil, p2p2Byte)
		sendMultiProposalTranscation(number, gen, saddr1, cert44, cert1, true, skey1, signer, statedb, fastChain, abiCA, nil, p2p2Byte)
		//sendDelCaCertTranscation(number, gen, saddr1, cert, skey1, signer, statedb, fastChain, abiCA, nil,p2p2Byte)
		sendGetCaCertAmountTranscation(number, gen, saddr1, cert, skey1, signer, statedb, fastChain, abiCA, nil, p2p2Byte)
	}
