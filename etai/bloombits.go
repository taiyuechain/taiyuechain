...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Taiyuechain
...
// for the Taiyuechain header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  etaidb.Database      // database instance to write index data and metadata into
...
func NewBloomIndexer(db etaidb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := etaidb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
