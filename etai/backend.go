...
func CreateDB(ctx *node.ServiceContext, config *Config, name string) (etaidb.Database, error) {
	db, err := ctx.OpenDatabase(name, config.DatabaseCache, config.DatabaseHandles)
	if err != nil {
		return nil, err
	}
	if db, ok := db.(*etaidb.LDBDatabase); ok {
...
func CreateConsensusEngine(ctx *node.ServiceContext, config *ethash.Config, chainConfig *params.ChainConfig,
	db etaidb.Database) consensus.Engine {
...
func (s *Taiyuechain) ChainDb() etaidb.Database           { return s.chainDb }
