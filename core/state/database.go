...
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb yuedb.Putter) error
}
...
func NewDatabase(db yuedb.Database) Database {
...
func NewDatabaseWithCache(db yuedb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb yuedb.Putter) error {
